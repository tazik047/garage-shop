  ...
  <td class="simplecheckout-table-form-right">
    ...
    <?php } elseif ($type == 'checkbox') { ?>
      <?php foreach ($values as $info) { ?>
        ...
        <label><input class="form-control" type="checkbox" name="<?php echo $name ?>[<?php echo $info['id'] ?>]" id="<?php echo $id ?>" value="1" <?php echo !empty($value[$info['id']]) ? 'checked="checked"' : '' ?> <?php echo $reload ? 'data-onchange="reloadAll"' : ''?>><?php echo $info['text'] ?></label>
      <?php } ?>
