...
//				error_log(print_r($this->session->data['payment_methods'],1));
	...
	public function shipping_method($render = true, &$data = array()) 
	{
		...
		if ($render !== false)	
		{
			...
			if ($opencart2) 
			{
				$this->response->setOutput($this->load->view('checkout/gn_shipping_method', $data));
			} else
			{
				$this->data = $data;
				$this->response->setOutput($this->render());
	...
	public function payment_address($render = true, &$data  = array()) {
  	...
  	public function payment_method($render = true, &$data = array()) {
		...
		if ($render !== false)
		{
			...
			if ($opencart2) 
			{
				$this->response->setOutput($this->load->view('checkout/gn_payment_method', $data));
			} else
			{
				$this->data = $data;
				$this->response->setOutput($this->render());
...
//				$data['shipping_method'] = $this->request->post['shipping_method'];				
			...
			foreach ($results as $result) {
				if ($this->config->get($result['code'] . '_status')) {
					...
					$this->{'model_extension_total_' . $result['code']}->getTotal($total_data1);
				}
			}
			...
			*/
			...
			if ($opencart2) 
			{
				$this->response->setOutput($this->load->view('checkout/gn_cart', $data));
			} else
			{
				$this->data = $data;
				$this->response->setOutput($this->render());			
