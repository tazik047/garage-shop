...
class ControllerCheckoutSimpleCheckoutCart extends SimpleController { 
    ...
    public function index() {
        ...
        if (!empty($minAmount) && $minAmount > $cartSubtotal) {
            $this->simplecheckout->addError();
            $this->simplecheckout->blockOrder(); 
            $this->_templateData['error_warning'] = sprintf($this->language->get('error_min_amount'),$this->currency->format($minAmount, $this->session->data['currency']));
        }
        
        if (!empty($maxAmount) && $maxAmount < $cartSubtotal) {
            $this->simplecheckout->blockOrder();    
            $this->simplecheckout->addError();
            $this->_templateData['error_warning'] = sprintf($this->language->get('error_max_amount'),$this->currency->format($maxAmount, $this->session->data['currency']));
        }
            ...
            foreach ($product['option'] as $option) {
                if ($version >= 200) {
                    ...
                    if ($option['type'] != 'file') {
                        ...
                        $option_value = $this->encryption->decrypt($option['option_value']);
        ...
        if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {                         
            ...
            foreach ($total_data as $key => $value) {
                ...
                if (!isset($value['text'])) {
                    $total_data[$key]['text'] = $this->currency->format($value['value'], $this->session->data['currency']);
                }
            }
        ...
        if ($currentTheme == 'shoppica' || $currentTheme == 'shoppica2') {
            $this->_templateData['cart_total'] = $this->currency->format($total, $this->session->data['currency']);
        } else {
            $this->_templateData['cart_total'] = sprintf($this->language->get('text_items'), $this->cart->countProducts() + (isset($this->session->data['vouchers']) ? count($this->session->data['vouchers']) : 0), $this->currency->format($total, $this->session->data['currency']));
        }
