...
class Simple {
    ...
    public static function getInstance($registry, $isAdminPage) {
    ...
    protected function __construct($registry, $isAdminPage = false) {
        ...
        $this->addLinksToStaticFiles($isAdminPage);
    ...
    public function addLinksToStaticFiles($isAdminPage) {
        ...
        $pref = $isAdminPage ? '' : 'catalog/';

        $this->document->addStyle($pref . 'view/' . $template . 'stylesheet/simple.css'.$version);

        $direction = $this->language->get('direction');

        if ($direction == 'rtl') {
            $this->document->addStyle($pref . 'view/' . $template . 'stylesheet/simple.rtl.css'.$version);
        }
        ...
        if (strpos($this->getAdditionalPath(), 'mijo') !== false) {
            ...
            $this->document->addScript($pref . 'view/javascript/jquery/ajaxupload.js');
        }

        if ($this->_opencartVersion >= 200) {
            $this->document->addScript($pref . 'view/javascript/jquery/ui/jquery-ui.min.js');
            $this->document->addStyle($pref . 'view/javascript/jquery/ui/jquery-ui.min.css');
        }
        $this->document->addScript('https://cdnjs.cloudflare.com/ajax/libs/angular.js/1.6.1/angular.min.js');

        //$this->document->addScript($pref . 'view/javascript/jquery/ui/i18n/jquery.ui.datepicker-'.$this->getCurrentLanguageCode().'.js');

        $this->document->addScript($pref . 'view/javascript/jquery/jquery-ui-timepicker-addon.js');

        $this->document->addScript($pref . 'view/javascript/jquery/jquery.maskedinput.min.js');

        $this->document->addScript($pref . 'view/javascript/easyTooltip.js');

        $this->document->addScript($pref . 'view/javascript/simple.js'.$version);
        ...
        if ($this->_page) {
            ...
            $this->document->addScript($pref . 'view/javascript/simple'.$page.'.js'.$version);
        }
    ...
    public function getCurrentLanguageCode() {
        if (empty($this->_data['language_code'])) {
            ...
            $this->_data['language_code'] = trim(strtolower($this->config->get('config_language')), '.');
        }
